...
test("template: from arrays", () => {
  ...
  assert.deepEqual(
    convert([1, null, "test", -1, undefined, NaN]),
    '[1, null, "test", -1, void 0, NaN]'
  )
})
...
test("template: from typed arrays", () => {
  ...
  assert.deepEqual(
    convert(new Uint8ClampedArray([1, 2, 3])),
    "new Uint8ClampedArray([1, 2, 3])"
  )
  assert.deepEqual(
    convert(new Int8Array([1, 2, 3])),
    "new Int8Array([1, 2, 3])"
  )
  assert.deepEqual(
    convert(new Uint16Array([1, 2, 3])),
    "new Uint16Array([1, 2, 3])"
  )
  assert.deepEqual(
    convert(new Int16Array([1, 2, 3])),
    "new Int16Array([1, 2, 3])"
  )
  assert.deepEqual(
    convert(new Uint32Array([1, 2, 3])),
    "new Uint32Array([1, 2, 3])"
  )
  assert.deepEqual(
    convert(new Int32Array([1, 2, 3])),
    "new Int32Array([1, 2, 3])"
  )
  assert.deepEqual(
    convert(new Float32Array([1, 2, 3])),
    "new Float32Array([1, 2, 3])"
  )
  assert.deepEqual(
    convert(new Float64Array([1, 2, 3])),
...
test("template: from array buffers", () => {
  ...
  assert.deepEqual(
    convert(new Uint8Array([1, 2, 3]).buffer),
