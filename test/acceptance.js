...
test("it lets you create an empty tree and append nodes to it", () => {
...
test("it lets you drop if statements", () => {
...
test("it lets you replace multiple nodes", () => {
...
test("it does not replace node when undefined is passed", () => {
  const ast = new AbstractSyntaxTree("if (false) { console.log(1); }")
  ast.replace({
    enter(node, parent) {
      if (node.type === "IfStatement" && node.test.value === false) {
...
test("it lets you calculate binary expressions", () => {
...
test("it lets you split multiple assignments", () => {
